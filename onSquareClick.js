...
import { selectPieceState } from "./selectPieceState.js";
...
export function onSquareClick(event) {
	...
	if (selectPieceState.pieceColor != Main.turnDecider) return; // same player can't move twice in a row
	...
	selectPieceState.selectedPieceArray = Main.pieceSquarePositionArray[selectPieceState.pieceColor][selectPieceState.pieceType];
	...
	selectPieceState.selectedPiece = Main.pieceElementsObject[selectPieceState.pieceColor][selectPieceState.pieceType][selectPieceState.selectedPieceIndex];
	...
	for (let i = 0; i < 64; i++) {
		...
		if (Main.stateGrid[i] < 0 && selectPieceState.pieceColor === 'black') { // 
			Main.grid[i].addEventListener('click', Main.moveToDestination);
	...
	Main.availablePieceMovesObject[selectPieceState.pieceType]();
}